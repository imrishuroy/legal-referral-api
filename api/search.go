...
func (server *Server) searchUsers(ctx *gin.Context) {
	...
	switch req.Filter {
	case "All":
		users, err := server.store.SearchAllUsers(ctx, req.Query)
		if err != nil {
	...
	case "1st":
		...
		users, err := server.store.Search1stDegreeConnections(ctx, args)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, errorResponse(err))
			return
		}
	...
	case "2nd":
		...
		users, err := server.store.Search2ndDegreeConnections(ctx, args)
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, errorResponse(err))
			return
		}
